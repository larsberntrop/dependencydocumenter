...
public class DependencyAnalyser {
	...
	public final boolean generateGraphvizlistAndConvertToGifFile() {
		...
		gv.writeGraphToFile(imageasBytes, getOutputPictureFileHandle());
		return true;
	 ...
	 */
	public final File getOutputPictureFileHandle() {
		if (tempPictureGif == null) {
			try {
				tempPictureGif = File.createTempFile("outputgif", ".gif");
				System.out.println(tempPictureGif);
	...
	public final boolean run() {
		...
		try {
			...
			try {
				// I'm loading the output of the dxl exporter into domDoc
	 ...
	 */
	final void scanDOMNodesandExtractDesignElements(final NodeList nodeList) {
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (children != null) {
				scanDOMNodesandExtractDesignElements(children);
			}
		}
	}
